...
def plot_histogram(tsv_file_path, column_name='id_ethnic_group_id'):
    """
    读取TSV文件并绘制指定列的直方图
    ...
    52: '鄂伦春族', 53: '赫哲族', 54: '门巴族', 55: '珞巴族', 56: '基诺族', 57: '其他', 58: '外国血统'
        ...
        # 绘制条形图
        bars = plt.bar(range(len(value_counts)), value_counts.values, alpha=0.7, color='#d2b48c', edgecolor='white')
        ...
        # plt.title('Ethnic Group Distribution', fontsize=14, fontweight='bold')
        
        # 设置横轴标签为种族名字，并去掉刻度线
        ethnic_names = [ethnic_mapping.get(int(x), f'ID {int(x)}') for x in value_counts.index]
        plt.xticks(range(len(value_counts)), ethnic_names, rotation=45, ha='center')
        ...
        # 保存超高像素图片（在显示之前保存）
        plt.savefig('histogram_ultra_high.png', dpi=1200, bbox_inches='tight', facecolor='white')
...
if __name__ == "__main__":
    ...
    # 绘制直方图
    plot_histogram(tsv_file, 'id_ethnic_group_id') 
